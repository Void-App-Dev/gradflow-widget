<!-- Container you can resize or style as you like -->
...
void main() {
  ...
  // Film-style grain (screen-space, per-frame jitter, FBM)
  if (u_noise > 0.001) {
    vec2 px = gl_FragCoord.xy;
    float base = u_noise * 0.25;           // overall intensity
    float lum = luma(color);
    float strength = base * mix(1.2, 0.6, lum); // a bit more in shadows
    float g = filmGrain(px, u_time, strength);
    color = max(color, 1e-3);
    color = exp(log(color) + g); // exposure-style modulation
  }
